#!/usr/bin/env node
/**
 * scripts/void-orphaned-picks-2026-05-30.js
 *
 * Companion to scripts/patch-card-2026-05-30.js. Finds picks whose
 * picked_fighter_id is no longer either side of their fight (orphaned
 * by tonight's late card swaps) and voids them: correct=NULL, points=0,
 * *_correct=NULL.
 *
 * Tonight's known orphan-creating swaps on event 105:
 *   - Fight 794: Salikhov (673) → Harris (877). Any pick on 794 with
 *     picked_fighter_id=673 is orphaned.
 *   - Fight 799: Aguilar (418) → Gurule (669). Any pick on 799 with
 *     picked_fighter_id=418 is orphaned.
 *
 * The detection query is corner-agnostic — it just compares
 * picked_fighter_id against the current red/blue ids of the fight,
 * so it stays correct if more swaps land.
 *
 * Without this fix, reconcilePicksForEvent would score those picks as
 * `correct=0` (a silent penalty for picking a fighter who was scratched).
 *
 ...
 *   node scripts/void-orphaned-picks-2026-05-30.js --dry-run
 *   node scripts/void-orphaned-picks-2026-05-30.js
 */
