 ...
 *   - official source URLs the reviewer should cross-check against
 *
 * IMPORTANT: this module does not write to the database. It does not generate
 * predictions or modify them. Live observations entered by the reviewer must
 * stay separate from `predictions.*` rows. The `live_checklist` field returns
 * a skeleton template for the UI to display; the server never persists it.
 */
...
function buildAudit(card) {
  ...
  future_enhancements.push('live observations are not persisted — keep model rows immutable post-fight');
...
async function buildPredictionReview({ db, eventId, officialDate }) {
  ...
  return {
    ...
    notes: {
      ...
      live_observations: 'NOT persisted; checklist is a UI skeleton only',
